...
function digraph(v)
	...
	for (n1, n2) in es
		push!(dot, "\"$(id(n1))\" -> \"$(id(n2))$(n2.op)\"")
	end
	string("digraph mygraph {\n", "rankdir=\"LR\"\n", join(dot, "\n"), "\n}")
end

# ╔═╡ ffef68df-dc5e-4ab8-99a4-c9347294f51b
plot_value(v::Value) = v |> digraph |> IOBuffer |> GraphViz.load

# ╔═╡ e43121a0-611e-48cf-8895-19ed08b8f1cf
md"""
Visualize a simple computaton:
"""

# ╔═╡ 74f5b9cc-cd60-41cf-b98d-3ca344f74cce
let
	x = Value(-4)
	y = Value(2)
	z = 4x^2 - 2y
...
# ╔═╡ 2aff0912-ce00-4737-af8e-d2a2d01b1340
md"""
### Dataset generator
"""

# ╔═╡ 183857f6-b108-4b95-81f3-3db55b169465
md"""

`make_moons` _adapted from_ [MLJBase](https://github.com/JuliaAI/MLJBase.jl/blob/29a6ef08acdad3c0bf6d0662bdc0a2aeef9c5e9c/src/data/datasets_synthetic.jl#L265):
"""
...
# ╔═╡ eff54fc7-d0eb-49c2-8b45-c404be7c3db9
gen_data = make_moons
...
# ╔═╡ 665744ba-638b-49fc-aeae-c0d725804de6
md"""
### Loss function
"""

# ╔═╡ 4ab11830-b349-4c95-8cc2-c40a4bf6651c
function loss(model, X, y, batch_size=nothing)
	# data batching
	if isnothing(batch_size)
		Xb, yb = X, y
	else
		ri = rand(1:size(X, 2), batch_size)
		Xb, yb = X[:, ri], getindex(y, ri)
	end

	# forward model to get scores
	scores = model.(@view Xb[:, i] for i in 1:size(Xb, 2))

	# svm "max-margin" loss
	losses = relu.(1 .- scores .* yb)
	data_loss = sum(losses) / length(losses)

	# L2 regularization
	alpha = 1e-4
	reg_loss = alpha * sum(p*p for p in parameters(model))

	# accuracy
	accuracy = [(yi > 0) == (scorei.v > 0) for (yi, scorei) in zip(yb, scores)]

	data_loss + reg_loss, sum(accuracy) / length(accuracy)
end

# ╔═╡ a2074f99-3c0c-4f1b-abbe-6f04ee12ec27
md"""
### Optimization loop
"""
...
function plot_descision_boundary(X, y, model, h=0.25)
	x_min, x_max = min.(X[1, :]...) - 1, max.(X[1, :]...) + 1
	y_min, y_max = min.(X[2, :]...) - 1, max.(X[2, :]...) + 1
	xrng = x_min:h:x_max
	yrng = y_min:h:y_max
	contourf(xrng, yrng, (x, y) -> model((x, y)).v > 0, legends=nothing)
	scatter!(
		X[1, :], X[2, :],
		markercolor=[x == 1 ? :red : :blue for x in y], legends=nothing
	)
...
md"""
Untrained model for comparisoin:
"""
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "09beb443bcdb559c6d0665ab4d619eb4851cd38d"
